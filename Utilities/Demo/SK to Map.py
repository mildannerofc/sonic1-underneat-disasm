...
with open('main.bin', 'rb') as f, open('output.asm', 'w') as o:
    ...
    for byte1, byte2 in zip(*[iter(open('main.bin', 'rb').read())]*2):
        replaced_byte1 = str(byte1 + 1) if byte1 + 1 <= 9 else f'${byte1 + 1:X}'
        replaced_byte2 = replace_map.get(byte2, str(byte2) if byte2 <= 9 else f'${byte2:X}'.lstrip('0') if byte2 < 0xA else f'${byte2:X}')
